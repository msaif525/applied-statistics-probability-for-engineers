...
$$y_i=\beta_0+\beta_1 x_i+\epsilon_i \hspace{10pt} ; i=1,2,3,..., N$$ {#eq-PRF}
...
$$E(y_i/x_i)=\beta_0+\beta_1 x_i $$ {#eq-expectedprf}
...
$$RSS=\sum_{i=1}^n e_i^2=\sum_{i=1}^n (y_i-\hat y_i)^2=\sum_{i=1}^n (y_i-\hat \beta_0-\hat \beta_1x_i)^2$$ {#eq-ssr}
...
$$\frac{\delta \hspace{1pt} (RSS)}{\delta \hat \beta_0}=0$$ {#eq-ols1}

$$\frac{\delta \hspace{1pt} (RSS)}{\delta \hat \beta_1}=0$$ {#eq-ols2}
...
$$\rho=\frac{\sigma_{XY}}{\sigma_X \sigma_Y} \ \ \ \ ; \ \ -1\le \rho \le +1$$ where, $\sigma_{X,Y}=Cov (X,Y)=E[(X-\mu_X)(Y-\mu_Y)]=E(XY)-\mu_X \mu_Y$
...
$$r_s=1-\frac{6\sum_{i=1}^n d_i^2}{n(n^2-1)}$$ {#eq-Rcc}
